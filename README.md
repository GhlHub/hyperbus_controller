...
- `0x0100` CK_P_ODELAY_CTRL (local, `PHY_IO_STYLE=IO_DELAY` only)
- `0x0104` CK_P_ODELAY_TIME (local, `PHY_IO_STYLE=IO_DELAY` only)
- `0x0108` CK_P_ODELAY_STATUS (local, `PHY_IO_STYLE=IO_DELAY` only)
- `0x01C0` RWDS_IDELAY_CTRL (local, `PHY_IO_STYLE=IO_DELAY` only)
- `0x01C4` RWDS_IDELAY_TIME (local, `PHY_IO_STYLE=IO_DELAY` only)
- `0x01C8` RWDS_IDELAY_STATUS (local, `PHY_IO_STYLE=IO_DELAY` only)
- `0x0200` DELAY_RST_CTRL (local, `PHY_IO_STYLE=IO_DELAY` only)
  - bit0 IDELAYCTRL reset request
  - bit1 CK_P ODELAY reset request
  - bit2 RWDS and DQ IDELAY reset request
  - bit3 external HyperRAM `RESET#` request
- `0x0204` IDELAYCTRL_STATUS (local, bit0 RDY, `PHY_IO_STYLE=IO_DELAY` only)
- `0x0300 + n*0x10` DQ`n`_IDELAY_CTRL for `n=0..7` (local, `PHY_IO_STYLE=IO_DELAY` only)
- `0x0304 + n*0x10` DQ`n`_IDELAY_TIME for `n=0..7` (local, `PHY_IO_STYLE=IO_DELAY` only)
- `0x0308 + n*0x10` DQ`n`_IDELAY_STATUS for `n=0..7` (local, `PHY_IO_STYLE=IO_DELAY` only)
...
- `0x0028` is no longer decoded by the AXI-Lite slave.
- HyperBus register-space access is explicitly decoded only for `ID0`, `ID1`,
  `CR0`, and `CR1`.
- Unmapped AXI-Lite addresses are handled locally with `OKAY` responses and
  zero read data.
